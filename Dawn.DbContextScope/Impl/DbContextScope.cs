...
namespace Dawn.DbContextScope.Impl
{
    public class DbContextScope : IDbContextScope
    {
        ...
        public void RegisterNew<TDbContext, TEntity>(TEntity entity)
           where TEntity : class where TDbContext : DbContext
        {
            _dbContexts.Get<TDbContext>().Set<TEntity>().Add(entity);
        }

        public void RegisterDirty<TDbContext, TEntity>(TEntity entity)
           where TEntity : class where TDbContext : DbContext
        {
            _dbContexts.Get<TDbContext>().Entry<TEntity>(entity).State = EntityState.Modified;
        }

        public void RegisterClean<TDbContext, TEntity>(TEntity entity)
           where TEntity : class where TDbContext : DbContext
        {
            _dbContexts.Get<TDbContext>().Entry<TEntity>(entity).State = EntityState.Unchanged;
        }

        public void RegisterDeleted<TDbContext, TEntity>(TEntity entity)
            where TEntity : class where TDbContext : DbContext
        {
            _dbContexts.Get<TDbContext>().Set<TEntity>().Remove(entity);
        }
...
" + Environment.StackTrace;
        ...
        internal static DbContextScope GetAmbientScope()
        {
            ...
            Console.WriteLine(System.Threading.Thread.CurrentThread.ManagedThreadId + " get : " + instanceIdentifier.GetHashCode());
